...
function renderGame() {
  ...
  if (gameWheel && gameBand) { gameBand.remove(); gameWheel.style.background = activePlayer?.role === 'Psychic' || state.revealed ? '#101614' : '#101614'; if (activePlayer?.role === 'Psychic' || state.revealed) { gameWheel.insertAdjacentHTML('afterbegin', '<div class="target-sector sector-one"></div><div class="target-sector sector-three-left"></div><div class="target-sector sector-five"></div><div class="target-sector sector-three-right"></div><div class="target-sector sector-one-right"></div>'); configureTargetWheel(gameWheel, state.target) } if (state.revealed) { Object.entries(state.guesses).forEach(([name, guessAngle]) => { const point = wheelPoint(guessAngle, .72); gameWheel.insertAdjacentHTML('beforeend', `<div class="guess-marker" style="left:${point.x}%;top:${point.y}%">${escapeHtml(name)} · ${guessAngle}°</div>`) }) } }
