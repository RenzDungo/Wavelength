...
const state: { screen: GameState; playerName: string; category: string; left: string; right: string; clue: string; code: string; round: number; totalRounds: number; target: number; guess: number; revealed: boolean; scores: number[]; players: Player[]; roomStatus: Room['status'] } = {
  screen: 'welcome', playerName: '', category: '', left: '', right: '', clue: '', code: '', round: 1, totalRounds: 3, target: 90, guess: 90, revealed: false, scores: [7, 4, 2, 0], players: [], roomStatus: 'lobby',
}
const escapeHtml = (value: string) => value.replace(/[&<>'"]/g, (character) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[character]!)
const syncRoom = (room: Room) => { state.players = room.players; state.category = room.category; state.left = room.left; state.right = room.right; state.clue = room.clue; state.target = room.target ?? state.target; state.round = (room.round ?? 0) + 1; state.totalRounds = room.totalRounds ?? state.totalRounds; state.roomStatus = room.status ?? 'lobby'; const currentGuess = room.guesses?.[state.playerName]; if (typeof currentGuess === 'number') state.guess = currentGuess }
...
const configureTargetWheel = (wheel: HTMLElement, target: number) => { const zones = wheel.querySelector<HTMLElement>('.target-zones'); if (zones) zones.style.transform = 'none'; const boundaries = [Math.max(0, target - 25), Math.max(0, target - 15), Math.max(0, target - 5), Math.min(180, target + 5), Math.min(180, target + 15), Math.min(180, target + 25)]; wheel.querySelectorAll<HTMLElement>('.target-sector').forEach((sector, index) => { const start = wheelPoint(boundaries[index]!); const end = wheelPoint(boundaries[index + 1]!); sector.style.clipPath = `polygon(50% 100%, ${start.x}% ${start.y}%, ${end.x}% ${end.y}%)` }); const labelDegrees = [Math.max(2, target - 20), Math.max(5, target - 10), target, Math.min(175, target + 10), Math.min(178, target + 20)]; wheel.querySelectorAll<HTMLElement>('.target-score').forEach((label, index) => { const point = wheelPoint(labelDegrees[index]!, .72); label.style.left = `${point.x}%`; label.style.top = `${point.y}%`; label.style.right = 'auto' }) }
function render() {
...
function renderGame() {
  ...
  const angle = state.guess + 180
    const guessDistance = Math.abs(state.guess - state.target)
    const score = state.revealed ? (guessDistance <= 5 ? 5 : guessDistance <= 15 ? 3 : guessDistance <= 25 ? 1 : 0) : null
